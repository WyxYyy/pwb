<template>
  <div class="login">
    ...
    <div class="Modular">
      ...
      <div class="ipt">
        <input :type="showconfirm_password?'text':'password'" v-model="confirm_password" placeholder="密码由6-16位数字，字母组成">
        <i @click="showconfirm_password = !showconfirm_password"
          :class="showconfirm_password?'noPassword':'showPassword'"></i>
      </div>
    </div>
    <div class="btns">
      <div class="btn" @click="confim">
        确 定
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      post() {
        ...
        this.$axios({
          'url': this.Interface.config.forgetPassword,
          'method': 'post',
          'data': postData
        }).then((res) => {
          if (res.data.code == 1) {
