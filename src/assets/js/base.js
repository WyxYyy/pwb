  ...
  exports.install = function (Vue, options) {
    Vue.prototype.wantTo =  function (where, data) {
     ...
     Vue.prototype.goToDetail = function (where,id){
        this.$router.push({
          path: `/`+where+`/${id}`,
        })
     }
    Vue.prototype.VerificationToken = function (res){
    ...
    // 微信分享
    Vue.prototype.wxShareClosure = function (w) {
      // 空函数
      function shield() {
        return false;
      }
    ...
    Vue.prototype.wxShareInit = function () {
      'use strict';
    ...
    Vue.prototype.getPhoneYZM = function(phone) {
      ...
      this.$axios({
        'url':'http://192.168.1.106/YueFuDaRen/public/api/user/send' ,
        'method': 'post',
        'data':phone,
      ...
      // 地址
    Vue.prototype.locationCity=''


    // APP的变量
    Vue.prototype.downloadPercent = 1;
    Vue.prototype.downloadStatus = false;
    //判断是否是APP
    Vue.prototype.downloadAndUpdateApp = function(url){

      var downloadPercent1 = 0
      // var interval = setInterval(function(){
      //   if(downloadPercent > 99){
      //     clearInterval(interval)
      //   }
      //   plus.nativeUI.closeWaiting();
      //   plus.nativeUI.showWaiting('下载中,请稍等!' + downloadPercent,{width:"100px",color:"#0f0"});
      //   console.log(downloadPercent)
      //   console.log('Vue百分比:' + Vue.prototype.downloadPercent)
      // },2000)
      var dtask = plus.downloader.createDownload(url,{}, function ( d, status ) {
        // 下载完成
        
        if ( status == 200 ) { //下载成功后的回调函数
            // plus.nativeUI.alert( "下载成功，去安装");
            console.log('下载成功，去安装');
            plus.nativeUI.confirm("下载成功，去安装", function(event) {
              if(event.index == 1) {
                plus.nativeUI.closeWaiting();
                plus.runtime.install('_downloads/pwb.apk',{},function(){
    ...
    dtask.addEventListener('statechanged', (task) => {
      ...
      downloadPercent1 = Math.ceil(task.downloadedSize / task.totalSize * 100);
      Vue.prototype.downloadPercent = downloadPercent1
      // plus.nativeUI.showWaiting('下载中,请稍等!',{width:"100px",color:"#0f0"});
      
      // Vue.prototype.downloadPercent = progressVal;
    ...
    Vue.prototype.getAppVersion = function (URL) {
        if(window.plus){
          // 这是app
          plus.runtime.getProperty(plus.runtime.appid,function(inf){
            //获取当前版本号
            var current_verison = inf.version;

            //从服务器获取最新版本
            var xhr=new XMLHttpRequest();
            // 'http://192.168.1.107/YueFuDaRen3/public/api/home/common/checkVersion'
            xhr.open('get',URL);
            xhr.onreadystatechange = function(){
              if(xhr.readyState==4&xhr.status==200){
                ...
                let new_verison = data.config

                let update_url = data.remark

                console.log("current_verison:" + current_verison)
                console.log("new_verison:" + new_verison)

                if(new_verison>current_verison)//比对版本号
                {
                    //console.log(new_version+'新版本'+version);
                    plus.nativeUI.confirm("应用有新版本，是否立即下载更新？", function(event) {
                    if(event.index == 1) {
                      // plus.nativeUI.showWaiting('下载中,请稍等!',{width:"100px",color:"#0f0"});
                      Vue.prototype.downloadStatus = true
                      Vue.prototype.downloadAndUpdateApp(update_url);//更新函数,在下面
                      }
                    }, '铺旺宝', ['取消', '确认']);
                }
                }
